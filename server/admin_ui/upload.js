...
(function (root, factory) {
   ...
   *
   * Лаунчер нумеруется строго тремя числами: его самообновление у игроков
   * отвергает номер с буквами (`SelfUpdateVersions`), и такая версия
   * оставила бы всех на старой. Игра нумеруется так, как её выпускает
   * разработчик, — 2.4.A, 0.8.12.19193: лаунчер сравнивает номер игры
   * по частям и число частей не ограничивает. Пустых частей («2..4»,
   * «2.4.») не бывает ни там, ни там.
   */
  const VERSION_RE = /^[A-Za-z0-9._-]+$/;
  const GAME_VERSION_RE = /^[A-Za-z0-9]+(?:[._-][A-Za-z0-9]+)*$/;
  function versionProblem(v, opts) {
    ...
    if (opts && opts.game) {
      if (!GAME_VERSION_RE.test(s)) return 'Между точками нужны цифры или буквы — например, 2.4.A';
      return '';
    }
    if (!/^\d+\.\d+\.\d+$/.test(s)) return 'Номер лаунчера — три числа через точку, например 1.6.47';
    return '';
  }
  ...
  function nextVersion(current) {
    ...
    // Номер игры в чужом формате: растёт последняя часть, ведущие нули
    // сохраняются (2.4.4.4.01 → 2.4.4.4.02), буква идёт следующей (2.4.A → 2.4.B)
    const num = /^(.*[._-])(\d+)$/.exec(v);
    if (num && GAME_VERSION_RE.test(v)) {
      return num[1] + String(Number(num[2]) + 1).padStart(num[2].length, '0');
    }
    const letter = /^(.*[._-])([A-Ya-y])$/.exec(v);
    if (letter && GAME_VERSION_RE.test(v)) {
