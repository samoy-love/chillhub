...
(function (root, factory) {
  ...
  function buildLog(events, state) {
    ...
    /* Прокручивается сам журнал, а не лист целиком: иначе подпись и
       кнопки уезжают вверх вместе со строками, а вернуться к ним посреди
       двадцатиминутной сборки надо чаще, чем кажется. */
    return '<div class="log scroll scroll--lg" data-log>' + list.map(logRow).join('') + '</div>';
  }
