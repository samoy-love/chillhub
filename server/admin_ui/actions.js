...
(function (root, factory) {
  ...
  const ACTIONS = {
    ...
    /* Удаление АКТИВНОЙ версии — не то же самое, что удаление старой.
       Сервер не убирает latest.json, а переставляет его на ближайшую
       предыдущую (`recalcLatest`), и все установленные лаунчеры уезжают
       на неё же — то есть откатываются. Вопрос обязан назвать это: в
       панели 1.0 он обещал обратное, и откат выглядел как то, чего
       никто не просил. */
    'launcher.delete': {
      danger: true,
      ask: (a) => ({
        ...
        body: a.active
          ? 'Это ПРЕДЫДУЩАЯ версия станет активной, и все установленные лаунчеры откатятся на неё при следующей проверке обновлений. Файлы удаляемой версии будут стёрты. Вернуть нельзя.'
          : 'Файлы версии будут стёрты с сервера. У игроков ничего не изменится: активной остаётся та же версия. Вернуть нельзя.',
        ok: 'Удалить',
      }),
    ...
    /* ПРАВИЛО ЧИСТКИ — СЕРВЕРНОЕ, И ОНО НЕ «ОСТАВИТЬ N СВЕЖИХ».
       Удаляется всё старше активной, кроме двух непосредственно перед
       ней: откатиться на шаг-два должно оставаться возможным, а всё
       новее активной — загруженное и ещё не отданное. Вопрос называет
       версии поимённо: «удалить старые» без списка — это просьба
       довериться, а доверяться тут нечему. */
    'launcher.prune': {
      danger: true,
      ask: (a) => ({
        title: 'Удалить ' + (a.victims || []).length + ' старых версий?',
        body:
          'Под нож: ' + (a.victims || []).join(', ') + '. Останутся активная ' + a.active +
          ', две перед ней и всё, что новее. Вернуть нельзя.',
        ok: 'Удалить старые',
      }),
      run: (api) => api.launcherPrune(),
      done: (a) => 'Удалено версий: ' + (a.victims || []).length,
      after: ['launcher'],
    },
    ...
    'news.publish': {
      ...
      run: (api, a) => api.newsPublish(a.scope || 'launcher', a.gameId || '', a.slug, a.published),
    ...
    'news.delete': {
      ...
      run: (api, a) => api.newsDelete(a.scope || 'launcher', a.gameId || '', a.slug),
      done: () => 'Заметка удалена',
      after: ['news', 'overview'],
    },
    'news.rebuild': {
      run: (api, a) => api.newsRebuild((a && a.scope) || 'launcher', (a && a.gameId) || ''),
    ...
    /* Чистить кэш по расписанию — значит платить временем каждой
       следующей сборки: те же архивы Thunderstore качались бы заново.
       Поэтому у чистки два разных повода и две разные кнопки. */
    'cache.sweep': {
      run: (api) => api.modsCacheSweep(),
      done: () => 'Просроченные архивы убраны',
      after: ['cache', 'disk'],
    },
    'cache.clear': {
      danger: true,
      ask: () => ({
        title: 'Очистить кэш архивов целиком?',
        body: 'Следующая сборка скачает все архивы заново — это минуты вместо секунд. Делать это стоит только когда кончается место.',
        ok: 'Очистить',
      }),
      run: (api) => api.modsCacheClear(),
      done: () => 'Кэш очищен',
      after: ['cache', 'disk'],
    },
    ...
    /* --- технические работы --- */
    /* Правка уже идущих работ — не то же самое, что их включение:
       спрашивать второй раз про то, что уже включено, незачем. */
    'maint.save': {
      run: (api, a) => api.maintenanceSet(a.payload),
      done: () => 'Сохранено. Игроки видят новый текст.',
