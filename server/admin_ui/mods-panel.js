  ...
  function createModsPanel(opts) {
    ...
    async function resolvePack(full, version) {
      ...
      try {
        ...
        // исчезало в том же тике, в котором появлялось. Поэтому строка
        // пишется здесь, а разблокировка идёт в finally без текста.
        setStatus(note);
        return plan;
      } catch (e) {
        say('Ошибка: ' + e, 'error');
        ...
        stop();
        // Разблокировка — только здесь. Пока она стояла в удачной ветке и в
        // catch, ответ сервера с не-2xx (Thunderstore недоступен) уходил через
        // `return null` мимо неё, и «Подтянуть», «Собрать», «Активировать»,
        // «Удалить» оставались серыми до перезагрузки страницы.
        setBusy(false);
        finishBuildCard();
      }
    }
    ...
    async function streamBuild(opts) {
      ...
      try {
        ...
        if (missing && !opts.allowMissing) {
          ...
          if (agreed) {
            // await, а не return: `return opts.retry()` внутри try выполнял
            // внешний finally сразу, как только вложенная сборка доходила до
            // первого await. Кнопки разблокировались, карточка прогресса
            // пряталась — и следующие двадцать минут сборка шла без единого
            // признака жизни, а панель позволяла запустить вторую такую же.
            await opts.retry();
            return;
          }
          return;
