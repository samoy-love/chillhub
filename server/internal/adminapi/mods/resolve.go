...
func (c *Client) resolveFrom(
	...
	//
	// Чужой пакет уходит вместе со своим поддеревом: раз его самого в сборке
	// нет, то и то, что нужно было ему одному, в ней не нужно.
	//
	...
	for depth := 0; len(frontier) > 0; depth++ {
		...
		for _, v := range fetched {
			...
			if rootFolder, ok := eco.LoaderRoot(v.Namespace, v.Name); ok {
				...
				//
				// Уходит он вместе со своим поддеревом: зависимости второго
				// загрузчика не обходятся вовсе. Это не экономия, а то же
				// правило: пакеты, которые тянет за собой BepInEx-BepInExPack,
				// — это его же ядро, и класть их рядом с ядром первого
				// загрузчика значит вернуть ту самую мешанину BepInEx/core.
				// Настоящему моду такая зависимость не принадлежит: он придёт
				// в дерево своим путём, от модпака.
				if res.Loader != "" {
