...
//
// Формат: блок — BlockSize байт подряд с начала файла, последний может быть
// короче. Хеш блока — SHA-256, обрезанный до blockDigestBytes байт. Хеши
// блоков файла склеены по порядку и записаны одной base64-строкой в поле
// "blocks"; размер блока объявлен один раз на манифест, в "blockSize".
//
...
// SHA-256 и Blake3 файла, как и раньше. Поэтому хватает двенадцати байт:
// случайное совпадение стоило бы одной лишней загрузки файла, а не подмены.
// Двенадцать байт кодируются в base64 ровно шестнадцатью символами, без
// выравнивания, — строка режется на блоки без разбора.
//
...
const (
	...
	// blockDigestBytes — сколько байт SHA-256 блока попадает в манифест.
	blockDigestBytes = 12
...
func (b *blockHasher) finishBlock() {
	sum := b.cur.Sum(nil)
	b.out = append(b.out, sum[:blockDigestBytes]...)
	b.cur.Reset()
	b.filled = 0
}
