...
func TestRecordInstallAndUpdate(t *testing.T) {
	...
	p.Record(Event{Event: "game_install", GameID: "kitty", Result: "ok", DurationMs: 60000, Bytes: 100, FullBytes: 100, FilesDownloaded: 10, FilesTotal: 10})
	p.Record(Event{Event: "game_install", GameID: "kitty", Result: "fail"})
	p.Record(Event{Event: "game_launch", GameID: "kitty"})

	mustContain(t, dump(t, reg),
		`chillhub_launcher_starts_total{app_version="1.4.0"} 1`,
		`chillhub_game_installs_total{game="kitty",result="ok"} 1`,
		`chillhub_game_installs_total{game="kitty",result="fail"} 1`,
		`chillhub_game_launches_total{game="kitty"} 1`,
		`chillhub_install_duration_seconds_sum{game="kitty"} 60`,
		`chillhub_downloaded_bytes_total{game="kitty"} 100`,
	)
}
...
func TestUpdateModeIsDerivedFromFileCounts(t *testing.T) {
	...
	p, reg := newProduct(t)
	p.Record(Event{Event: "game_update", GameID: "kitty", Result: "ok", FilesDownloaded: 3, FilesTotal: 100, Bytes: 5, FullBytes: 500})
	p.Record(Event{Event: "game_update", GameID: "kitty", Result: "ok", FilesDownloaded: 100, FilesTotal: 100, Bytes: 500, FullBytes: 500})
	p.Record(Event{Event: "game_update", GameID: "kitty", Result: "fail"})

	mustContain(t, dump(t, reg),
		`chillhub_game_updates_total{game="kitty",result="ok",mode="diff"} 1`,
		`chillhub_game_updates_total{game="kitty",result="ok",mode="full"} 1`,
		`chillhub_game_updates_total{game="kitty",result="fail",mode="unknown"} 1`,
		`chillhub_downloaded_bytes_total{game="kitty"} 505`,
		`chillhub_build_full_bytes_total{game="kitty"} 1000`,
		`chillhub_downloaded_files_total{game="kitty"} 103`,
		`chillhub_build_files_total{game="kitty"} 200`,
	)
}

func TestIntegrityAndErrors(t *testing.T) {
	p, reg := newProduct(t)
	p.Record(Event{Event: "integrity_check", GameID: "kitty", Result: "fail", HashMismatches: 4})
	p.Record(Event{Event: "integrity_check", GameID: "kitty", Result: "ok"})
	p.Record(Event{Event: "error", ErrorCode: "sync_io"})
	p.Reject("bad_body")

	mustContain(t, dump(t, reg),
		`chillhub_integrity_checks_total{game="kitty",result="fail"} 1`,
		`chillhub_integrity_checks_total{game="kitty",result="ok"} 1`,
		`chillhub_hash_mismatches_total{game="kitty"} 4`,
...
func TestHostileLabelsAreFolded(t *testing.T) {
	...
	p, reg := newProduct(t)
	p.Record(Event{Event: "game_launch", GameID: `kitty" or 1=1`})
...
func TestSubmitFeedsCounters(t *testing.T) {
	...
	body := map[string]any{
		"event": "game_update", "gameId": "kitty", "result": "ok",
	...
	mustContain(t, dump(t, reg),
		`chillhub_game_updates_total{game="kitty",result="ok",mode="diff"} 1`,
		`chillhub_downloaded_bytes_total{game="kitty"} 7`,
		`chillhub_build_full_bytes_total{game="kitty"} 700`,
...
func TestSubmitClampsFileCounts(t *testing.T) {
	...
	mustContain(t, dump(t, reg),
		`chillhub_build_files_total{game="kitty"} 10000000`,
		`chillhub_hash_mismatches_total{game="kitty"} 10000000`,
	)
	if strings.Contains(dump(t, reg), `chillhub_downloaded_files_total{game="kitty"} -`) {
