...
namespace ChillHub.Core {
    ...
    public class AppConfig {
        ...
        // Теперь потолок заведомо выше любого домашнего канала, и любое значение,
        ...
        // (см. Core/Metrics/MetricsService.cs и Core/Diagnostics.cs), а выключенный
        // у одного игрока отчёт означал ровно одно: сломавшийся у него запуск
        // никто не увидит. Переменная окружения CHILLHUB_METRICS=0 осталась —
        // ею пользуются тесты и отладочные прогоны.
        //
        // Старые config.json могут содержать эти ключи; они просто игнорируются.
        ...
        // Версия, для которой уже показали список обновлений. Пусто — не показывали ни разу.
        // Хранится номер, а не флаг: после следующего обновления список надо показать снова,
        // а после отката на старую сборку — нет.
