    ...
    internal static class ModsService {
        ...
        internal static async Task<ModsSyncResult> EnsureAsync(
            ...
            try {
                ...
                //
                // Занятый файл движок не заменяет: он кладёт новое содержимое рядом
                // как «.new» и планирует замену на перезагрузку, а на диске остаётся
                // старое. Запиши мы после этого маркер версии — и лаунчер объявил бы
                // модпак готовым: пункт «Steam · с модами» становится «Играть», тост
                // говорит «Моды установлены», а игра идёт с половиной пака. Случай не
                // редкий: копию из Steam запускают Steam'ом, мимо лаунчера, и сделать
                // это можно прямо посреди установки, которая идёт минутами.
                if (plan.DeferredToReboot.Count > 0) {
                    Logging.Logger.Warn(
                        $"[mods] модпак '{mods.Version}' применён не полностью: " +
                        $"{plan.DeferredToReboot.Count} файл(ов) заняты и заменятся после перезагрузки");
                    Metrics.MetricsService.Error("mods_reboot_required", game.GameId);
                    return new ModsSyncResult(
                        ModsSyncOutcome.Failed, mods.Version, plan.TotalDownloadBytes, toDelete, RebootMessage());
                }
        ...
        /// <summary>
        /// Что сказать, когда файлы модов заняты и заменятся только после перезагрузки.
        /// <para>
        /// Первым делом — закрыть игру: почти всегда держит файлы именно она, и тогда
        /// повтор помогает сразу, без перезагрузки. Перезагрузка названа вторым шагом,
        /// потому что уже скачанное никуда не денется — замена запланирована.
        /// </para>
        /// </summary>
        /// <returns>Текст для игрока.</returns>
        internal static string RebootMessage() =>
            "Файлы модов заняты другой программой. Закройте игру и повторите, " +
            "а если не поможет — перезагрузите компьютер.";
