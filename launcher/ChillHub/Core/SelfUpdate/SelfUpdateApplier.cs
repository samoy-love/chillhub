...
namespace ChillHub.Core.SelfUpdate {
    ...
    internal sealed class SelfUpdateApplier {
        ...
        /// но реальный рантайм (hostfxr.dll и т.п.) при этом оставался только в общей папке
        /// установки — скопированный в изоляцию для самообновления, апдейтер падал мгновенно,
        /// до единой строки в своём журнале (эта строка попадала не в apply-update.log, а в
        /// Windows Event Log: 'hostpolicy.dll' ... not found). В общей папке установки такой
        /// апдейтер выглядел рабочим, поэтому баг не был виден до самого самообновления.
        /// </para>
        /// <para>
        /// Глоб ниже по-прежнему копирует всё, что найдёт (включая .dll/.deps.json/
        /// .runtimeconfig.json — они остаются рядом с ChillHub.exe, поскольку сам ChillHub
        /// ссылается на типы из сборки апдейтера как на библиотеку). Эти файлы просто больше
        /// не нужны для запуска и ничего не портят своим присутствием. Проверка целостности
        /// копии (совпадение размера) защищает от усечённой копии .exe — раньше это защищало
        /// от рухнувшего apphost, теперь — от повреждённого нативного бинаря; смысл тот же:
        /// лучше не гасить приложение, чем запустить заведомо нерабочую копию.
        /// </para>
        /// </summary>
