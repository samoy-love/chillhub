            ...
            switch (p.Stage) {
                ...
                case "Downloading":
                    if (p.TotalBytes > 0) {
                        ...
                        // Сколько из сделанного пришло по сети — пока разница заметна.
                        var overNetwork = p.NetworkBytes > 0 && p.NetworkBytes < p.BytesDownloaded * 0.9
                            ? $" • по сети {FormatSize(p.NetworkBytes)}"
                            : string.Empty;
                        return new SyncProgressDisplay(
                            what + "Скачивание…",
                            false,
                            ...
                            $"{p.FilesDownloaded}/{p.TotalFiles} • {FormatSize(p.BytesDownloaded)}/{FormatSize(p.TotalBytes)}{overNetwork}");
                    }

                    return new SyncProgressDisplay(what + "Скачивание…", false, null, null, null);
                case "Verifying":
                    return new SyncProgressDisplay(what + "Проверка скачанного…", true, 100, string.Empty, null);
                case "Activating":
                    return new SyncProgressDisplay(what + "Применение…", true, 100, string.Empty, null);
                case "Completed":
                    return new SyncProgressDisplay(what + "Готово", false, 100, string.Empty, null);
                default:
