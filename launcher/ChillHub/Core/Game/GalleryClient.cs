...
namespace ChillHub.Core.Game {
    ...
    public sealed class GalleryClient {
        ...
        /// <summary>
        /// Забывает всё, что уже разобрано.
        /// <para>
        /// Кеш живёт весь процесс и ключуется только идентификатором игры: обложка,
        /// заменённая в админке, не доезжала до витрины до перезапуска лаунчера —
        /// «Обновить список игр» перезапрашивал сам список, но не галереи. Зовётся
        /// явным действием пользователя, а не в фоне: иначе каждый пересчёт витрины
        /// тянул бы `gallery.json` всех игр заново.
        /// </para>
        /// </summary>
