...
namespace ChillHub.Core.Game {
    ...
    internal sealed class GameSyncRunner {
        ...
        internal async Task RunAsync(GameSyncRequest request, CancellationToken token) {
            ...
            DiffPlan? plan = null;
            try {
                // Первый шаг объявляется сам, до того как за него возьмутся. Опрос
                // процессов и первое обращение к папке игры на спящем диске занимают
                // заметное время, а строка всё это время оставалась на той фразе, которую
                // поставил вызывающий («начинаем…»): отличить работу от зависания по ней
                // было нельзя.
                this.ui.SetStatus("Проверяем, не запущена ли игра…");
