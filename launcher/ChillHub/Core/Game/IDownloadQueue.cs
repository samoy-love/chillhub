...
namespace ChillHub.Core.Game {
    ...
    /// трафиком в 2 ГБ, и без этого числа полоса выглядит как «качаю 49 ГБ».
    /// </param>
    /// <param name="WorkBytesPerSecond">
    /// Сглаженная скорость работы — сделанные байты, а не сетевые. По ней считается
    /// остаток времени: делить оставшуюся работу на скорость сети неверно, когда
    /// большая её часть берётся с диска.
    /// </param>
    /// <param name="Cancelling">
    /// Позицию попросили остановить, но движок ещё не встал. Отдельный признак, а не
    /// состояние: пока идёт остановка, позиция по-прежнему занимает воркер, и снимать её
    /// из списка рано. Экрану он нужен, чтобы не выдавать останавливаемую закачку за
    /// идущую и чтобы кнопка предлагала запустить заново, а не отменять ещё раз.
    /// </param>
    /// <summary>
