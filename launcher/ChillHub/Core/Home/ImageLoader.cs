...
namespace ChillHub.Core.Home {
    ...
    internal static class ImageLoader {
        ...
        /// <summary>
        /// Сбрасывает кеш готовых картинок целиком.
        /// <para>
        /// Кеш живёт весь сеанс и ключуется только URL — если сервер отдаёт новую обложку
        /// по тому же адресу (обычная практика для манифестов/CDN), <see cref="LoadImageAsync"/>
        /// находил старые байты в кеше и никогда не ходил в сеть за новыми: у пользователя
        /// «Обновить список игр» ничего не менял в обложках. Вызывается явным действием
        /// пользователя (обновление списка игр), а не тихо в фоне — иначе каждая перерисовка
