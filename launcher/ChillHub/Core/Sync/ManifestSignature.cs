...
namespace ChillHub.Core.Sync {
    ...
    public static class ManifestSignature {
        ...
        /// <para>
        /// Заполнен: пара выпущена на сервере, приватная часть лежит в
        /// <c>MANIFEST_SIGNING_KEY</c> (файл <c>/etc/chillhub/manifest-signing.env</c>,
        /// права 0600) и в репозиторий не попадала. Смена ключа требует выпуска
        /// новой версии лаунчера — старые версии проверяют старым ключом.
        /// </para>
        /// <para>
        /// Пересоздать пару: <c>go run ./internal/adminapi/builds/keygen</c> в
        /// каталоге <c>server</c>; приватную часть — на сервер, публичную — сюда.
        /// </para>
        /// <para>
