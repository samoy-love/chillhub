...
namespace ChillHub.Core.Sync {
    ...
    public class DiffPlan {
        ...
        /// <summary>
        /// Gets or sets сколько места занимают ПРЯМО СЕЙЧАС те файлы, которые обновление
        /// заменит.
        /// <para>
        /// Разница с <see cref="TotalDownloadBytes"/> и есть реальный прирост занятого
        /// места: заменяемый файл не прибавляется к сборке, а подменяется, и его старые
        /// байты освобождаются, как только новая версия встала на место. Пока файлы
        /// качались в staging, это было неважно — вторая копия лежала на диске целиком,
        /// — но теперь загрузка идёт сразу на место, и требовать свободным весь объём
        /// загрузки значит отказывать в обновлении, которое спокойно поместилось бы.
        /// </para>
        /// </summary>
        public long ReplacedBytes { get; set; }
