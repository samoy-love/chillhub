    ...
    public partial class App : Application
    {
        protected override void OnStartup(StartupEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    try { File.AppendAllText(GetBootLogPath(), $"[" + DateTime.Now.ToString("o") + $"] UnhandledException: {ex.ExceptionObject}\r\n"); } catch { }
                    try { Console.Error.WriteLine($"[FATAL] UnhandledException: {ex.ExceptionObject}"); } catch { }
                    try { Logger.Error("UnhandledException: " + ex.ExceptionObject); } catch { }
                    MessageBox.Show($"Необработанное исключение: {ex.ExceptionObject}", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
                };
                this.DispatcherUnhandledException += (s, ex) =>
                {
                    try { File.AppendAllText(GetBootLogPath(), $"[" + DateTime.Now.ToString("o") + $"] DispatcherUnhandledException: {ex.Exception.Message}\r\n{ex.Exception}\r\n"); } catch { }
                    try { Console.Error.WriteLine($"[ERROR] {ex.Exception}"); } catch { }
                    try { Logger.Error(ex.Exception, "DispatcherUnhandledException"); } catch { }
        ...
        {
            try { File.AppendAllText(GetBootLogPath(), "[" + DateTime.Now.ToString("o") + "] Starting Application_Startup\r\n"); } catch { }
            try { Console.WriteLine("[BOOT] Starting Application_Startup"); } catch { }
            ...
            try { File.AppendAllText(GetBootLogPath(), "[" + DateTime.Now.ToString("o") + "] Showing UpdateWindow\r\n"); } catch { }
            try { Console.WriteLine("[BOOT] Showing UpdateWindow"); } catch { }
            var ok = upd.ShowDialog() == true || upd.Proceed;
            try { File.AppendAllText(GetBootLogPath(), "[" + DateTime.Now.ToString("o") + $"] UpdateWindow result ok={ok}\r\n"); } catch { }
            try { Console.WriteLine($"[BOOT] UpdateWindow result ok={ok}"); } catch { }
            if (!ok)
            {
                // Пользователь закрыл окно или обновление обязательно
                try { File.AppendAllText(GetBootLogPath(), "[" + DateTime.Now.ToString("o") + "] Shutting down after update dialog\r\n"); } catch { }
                try { Console.WriteLine("[BOOT] Shutting down after update dialog"); } catch { }
                Shutdown();
                return;
            }
            ...
            try { File.AppendAllText(GetBootLogPath(), "[" + DateTime.Now.ToString("o") + "] Showing MainWindow\r\n"); } catch { }
            try { Console.WriteLine("[BOOT] Showing MainWindow"); } catch { }
            mw.Show();
        }
        ...
        private static string GetBootLogPath()
        {
            ...
            catch { return Path.Combine(Environment.CurrentDirectory, "boot.log"); }
        }
