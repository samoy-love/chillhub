...
- Если у изменённого файла в манифесте есть `blocks`, а на месте лежит его старая
  копия, новый файл собирается в `.part` из блоков старой копии (поиск по хешу, на
  любом смещении — сдвинутые внутри pak-архива данные находятся) и недостающих
...
- Замер на Bodycam 1.0.0 → 0.8.12.19193: целиком — 52.9 ГБ, по блокам — ~2.7 ГБ.
