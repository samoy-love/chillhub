...
- Хеши блоков — для обновления по блокам ([9.3.1](#931-обновление-по-блокам)). Блок — `blockSize` байт подряд с начала файла (сейчас 1 МиБ), последний короче. На блок 16 байт: скользящий хеш (Σ b[i]·P^(n−1−i) по модулю 2^64, P = 0x100000001B3, little-endian) и первые 8 байт SHA-256; всё склеено по порядку в одну base64-строку `blocks`. Пишутся в том же проходе по файлу, только файлам больше одного блока; манифест без таких файлов не содержит ни `blocks`, ни `blockSize`. Формат закреплён одной и той же эталонной строкой в тестах сервера (`blocks_test.go`) и клиента (`BlockDeltaTests.cs`). Несогласованные блоки сервер не публикует (`validateManifest`), клиент — не использует, но манифест не отвергает.
- Манифестам, выпущенным до блоков, их дописывает `chillhub-admin backfill-blocks [-game id] [-version v]` — запуск на сервере от владельца контента с тем же `CONTENT_ROOT`, что у службы. Каждый файл сверяется с полными хешами манифеста, при расхождении манифест не трогается; манифест, переписанный за время подсчёта, не затирается. Повторный запуск ничего не меняет. Лаунчер пропускается: себя он обновляет без блоков.
...
- Загрузка версии (ZIP):
  ...
  - Чанковая загрузка (используется UI для многогигабайтных сборок; только `/admin/api/...`):
    ...
    (`StartUploadJanitor`).
  - `kind` в `upload/init`: `launcher`, `game` (пустой тоже значит `game`) или `mods`;
    иное отвергается с 400. `mods` публикует архив в ветку `_mods` рядом с собранными
    с Thunderstore версиями (см. [8.8](#88-моды)); `latest.json` не трогается.
  - В панели: «Игры» → карточка игры → «Версии» → «Залить версию»; «Сборки модов» →
    «Готовый архив». Номер вводится до выбора файла, поверх версии у игроков панель не
    заливает. После разбора архива лист предлагает «Отдать игрокам» с подтверждением.
...
Галерея игры (`server/internal/adminapi/gamegallery`, лист «Галерея» в разделе «Игры»):
...
- Готовый архив модпака заливается чанковой загрузкой с `kind=mods`
  (см. [8.1](#81-игры)). Записи `sources/{version}.json` у такой версии нет:
  `mods/list` отдаёт её с `kind: "upload"`, а `mods/diff` отвечает, что состав по
  пакетам неизвестен.
...
### 9.3.1. Обновление по блокам

`Core/Sync/BlockDelta.cs`, `Core/Sync/BlockList.cs`, встроено в `SimpleSyncService.ExecuteAsync`.

- Если у изменённого файла в манифесте есть `blocks`, а на месте лежит его старая
  копия, новый файл собирается в `.part` из блоков старой копии и недостающих
  кусков, скачанных Range-запросами (соседние блоки — одним запросом, до 32 блоков).
- Блоки в старой копии ищутся на любом смещении (`Core/Sync/BlockFinder.cs`, схема
  zsync): окно размером в блок катится по старому файлу с шагом в байт, сдвиг
  скользящего хеша — одно умножение, SHA-256 окна считается только при совпадении.
  Так находятся и данные, сдвинутые на мегабайты внутри pak-архива Unreal, и файлы
  Unity, подросшие на сотню байт. После совпадения окно прыгает на блок вперёд.
  Больше 1024 совпадений хеша при другом SHA-256 — дальше по файлу шаг в блок
  (защита от худшего случая).
- Замер (к загрузке): PEAK 2.4.1 → 2.4.3 — целиком 4.28 ГБ, по выровненным блокам
  4.25, скользящим окном 2.49; Bodycam 1.0.0 → 0.8.12.19193 — 52.91, 2.64 и 2.61.
- Каждый блок пишется только после сверки с хешем из манифеста, поэтому `.part`
  всегда — проверенное начало нового файла. Уцелевший `.part` сверяется поблочно и
  продолжается; чужой обрезается по первому несовпадению.
- Собранный файл принимается только по полным SHA-256/Blake3, как скачанный целиком.
- Любой сбой сборки (раздача без Range, не тот кусок, испорченные байты, обрывы
  сверх трёх подряд без продвижения, занятый файл) — не ошибка: файл докачивается
  обычным путём с проверенного начала `.part`. Файл из соседней копии игры (донор)
  берётся раньше сборки по блокам.
- Чтение старых копий ограничено четырьмя файлами одновременно (как проверка файлов).
- Полные хеши собранного файла считаются по ходу записи — файл не перечитывается.
- `.part` пишется сквозной записью (`FileOptions.WriteThrough`). Обычная запись при
  копировании со скоростью диска набивает кеш Windows, система останавливает пишущих
  до сброса, и отмена ждала по 20–40 с. Скорость та же (замер на Bodycam: 405 с
  против 390 с на холодном кеше), отмена — меньше секунды.
- Взятое из старых копий (`DiffPlan.BlockReusedBytes`) в метрику скачанного не входит.
...
- Публикация новой версии игры:
  ...
  2) В панели, в разделе «Игры», выбрать `gameId` и `version` и загрузить ZIP — при необходимости с отметкой «Обновить latest».
