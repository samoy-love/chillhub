...
Оттуда они и доезжают до сервера: `BUILD_CMD` обеих серверных целей кладёт
`deploy/systemd/<юнит>` и `<юнит>.d/` в артефакт, а deploy-kit ставит их в
`/etc/systemd/system` и перезапускает службу. До 04.09.2026 этого не было —
файлы лежали в репозитории, но выкаткой не возились, и правку приходилось
доставлять руками. Выглядело это как зелёная выкатка, после которой на проде
