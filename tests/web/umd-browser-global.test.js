// Проверяет, что модули панели реально работают в том режиме, ради которого
// их UMD-обёртка и написана: обычным <script> в браузере, без CommonJS.
// require() в остальных tests/web/*.test.js всегда идёт по ветке
// `module.exports` и никогда не исполняет `Object.assign(root, factory())` —
// этот тест исполняет исходники в vm-контексте без `module`, то есть ровно в
// том окружении, где они работают на странице: window есть, module — нет.
//
// filename в vm.runInContext — не для галочки: с ним V8 связывает исполнение
// с настоящим файлом, и покрытие считается по строкам исходника.
