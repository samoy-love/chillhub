...
// ---- Время обращений ----
// Сама разметка времени переехала в admin-time.js и проверяется там: панель
// показывает одно время во всех вкладках, и держать для обращений отдельную
// проверку значило бы описывать несуществующее отличие. Здесь остаётся только
// то, что admin.js действительно зовёт общий форматтер, а не завёл свой.
test('время обращений берётся из общего форматтера панели', () => {
  assert.match(src, /const fbFmtTime = \(v\)=> window\.formatMoscow\(v\);/);
  assert.doesNotMatch(src, /Intl\.DateTimeFormat/,
    'разметка времени живёт в admin-time.js — вторая копия разъедется с первой');
});
