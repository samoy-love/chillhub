...
| `MANIFEST_SIGNING_KEY` | — | приватный ключ Ed25519 (base64) для подписи манифестов. Без него манифесты публикуются БЕЗ подписи и в журнал пишется предупреждение. Генерация пары: `cd server && go run ./internal/adminapi/builds/keygen`. Подробности — `Documentation.md` §5.1 |
