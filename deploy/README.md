...
# другой образ / другая версия:
NGINX_IMAGE=nginx:1.25-alpine sh deploy/nginx-check.sh
```
...
Требования: установленный Docker. Образ по умолчанию — `nginx:alpine`
(≥ 1.25, что обязательно для директивы `http2 on;`). Проверено на 1.25.5 и
1.31.3 — обе версии принимают конфиг без ошибок и предупреждений.
