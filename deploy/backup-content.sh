...
# Ротация ниже их не трогает: она исключает суффикс .FAILED явно.
...
#
# `! -name '*.FAILED'` обязателен: имя неудачного снимка — это та же отметка
# времени с суффиксом, то есть под '20*' оно подпадает тоже. Без исключения
# ротация считала неудачные снимки своими и удаляла их по своему лимиту —
# успешных при BACKUP_KEEP=14 хранилось на один меньше, а диагностический
# каталог исчезал раньше, чем до него доходили руки.
mapfile -t snaps < <(find "$BACKUP_ROOT" -mindepth 1 -maxdepth 1 -type d -name '20*' ! -name '*.FAILED' -printf '%f\n' | sort)
total=${#snaps[@]}
